...
// The Broadcaster home has its own latest-honours composition (bcHonoursRow).
// In that case the feed/state above is still required, but inserting the legacy
// honours strip again would duplicate content and has no compatible anchor.
const broadcasterOwnsHonours = app.includes('bcHonoursRow') || app.includes('AJPA BROADCASTER HOME V1');
if (!app.includes('s.honoursWrap') && !broadcasterOwnsHonours) {
