"""Load West Ham United from data/West Ham United.json with full PES6 stats and AJAP OVR."""
...
COUNTRY = "Inglaterra"
MIGRATION_MARKER = "west_ham_united_json_v1_ovr3_20260827"
SOURCE = "West Ham United.json • OVR AJPA promedio de 3 stats • 2026-08-27"
SOURCE_PATH = Path(__file__).resolve().parent / "data" / "West Ham United.json"
...
def _load_source():
    ...
    if str(payload.get("equipo", "")).strip().casefold() != WEST_HAM_UNITED.casefold():
        raise ValueError("West Ham United.json no corresponde a West Ham United")
    ...
    if len(players) != 26 or set(by_name) != set(POSITION_BY_PLAYER):
        ...
        raise ValueError(
            f"West Ham United.json inválido: jugadores={len(players)} faltan={missing or '-'} sobran={extra or '-'}"
        )
    return by_name


WEST_HAM_UNITED_DATA = _load_source()
...
def _role_values(position, stats):
    ...
    if values is None:
        raise RuntimeError(f"Posición West Ham United sin fórmula OVR: {position}")
...
WEST_HAM_UNITED_ROSTER = [
    (name, POSITION_BY_PLAYER[name], _rating(POSITION_BY_PLAYER[name], WEST_HAM_UNITED_DATA[name]["stats"]))
    for name in POSITION_BY_PLAYER
]

# Make West Ham United visible in the team selector before Discord views are registered.
if not any(name.casefold() == WEST_HAM_UNITED.casefold() for name, _country in teams.OFFICIAL_TEAMS):
    teams.OFFICIAL_TEAMS.append((WEST_HAM_UNITED, COUNTRY))
teams.OFFICIAL[WEST_HAM_UNITED.casefold()] = WEST_HAM_UNITED
...
def _sync_connection(runtime, conn):
    ...
    names = [name for name, _position, _rating_value in WEST_HAM_UNITED_ROSTER]
    marks = ",".join("?" for _ in names)
    present = conn.execute(
        f"SELECT COUNT(*) AS n FROM roster_players WHERE name COLLATE NOCASE IN ({marks})",
        tuple(names),
    ).fetchone()
    if int(present["n"] if present else 0) < len(WEST_HAM_UNITED_ROSTER):
    ...
    if done:
        conn.execute(
            ...
            (WEST_HAM_UNITED, COUNTRY),
        )
        return 0

    changed = 0
    for name, position, rating in WEST_HAM_UNITED_ROSTER:
        row = conn.execute(
        ...
        if row:
            ...
            conn.execute(
                ...
                (position, WEST_HAM_UNITED, rating, minimum_for_rating(rating), player_id),
            )
        else:
            cursor = conn.execute(
                ...
                (name, position, WEST_HAM_UNITED, rating, minimum_for_rating(rating)),
            )
            player_id = int(cursor.lastrowid)
        _upsert_attributes(conn, player_id, WEST_HAM_UNITED_DATA[name])
        changed += 1
    ...
    if "deleted_teams" in _tables(conn):
        conn.execute("DELETE FROM deleted_teams WHERE name = ? COLLATE NOCASE", (WEST_HAM_UNITED,))
    conn.execute("INSERT INTO seed_state (key) VALUES (?)", (MIGRATION_MARKER,))
    return changed


def apply_west_ham_united_json(runtime):
    if getattr(runtime, "_ajap_west_ham_united_json", False):
        return
    base_db = runtime.db
    synced_guilds = set()

    def west_ham_united_synced_db():
        conn = base_db()
        ...
        try:
            ...
            if changed:
                print(f"AJAP West Ham United cargado: guild={guild_id} • 26 jugadores desde West Ham United.json")
        except Exception:
            conn.close()
            raise
        return conn

    runtime.db = west_ham_united_synced_db
    runtime._ajap_west_ham_united_json = True
    print("AJAP migración West Ham United activa: 26 jugadores • OVR AJPA de 3 stats")


_original_apply_guild_isolation_patch = guild_isolation.apply_guild_isolation_patch


def _apply_guild_isolation_then_west_ham_united(runtime, bot):
    _original_apply_guild_isolation_patch(runtime, bot)
    apply_west_ham_united_json(runtime)


if not getattr(guild_isolation.apply_guild_isolation_patch, "_ajap_west_ham_united_json_wrapped", False):
    _apply_guild_isolation_then_west_ham_united._ajap_west_ham_united_json_wrapped = True
    guild_isolation.apply_guild_isolation_patch = _apply_guild_isolation_then_west_ham_united


OVR_BY_PLAYER = {name: rating for name, _position, rating in WEST_HAM_UNITED_ROSTER}
print(f"AJAP West Ham United JSON listo: {len(WEST_HAM_UNITED_ROSTER)} jugadores")
