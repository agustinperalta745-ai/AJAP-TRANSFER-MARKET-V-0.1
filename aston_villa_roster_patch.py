"""Load Aston Villa from data/Aston Villa.json with full PES6 stats and AJAP OVR."""
...
COUNTRY = "Inglaterra"
MIGRATION_MARKER = "aston_villa_json_v1_ovr3_20260827"
SOURCE = "Aston Villa.json • OVR AJPA promedio de 3 stats • 2026-08-27"
SOURCE_PATH = Path(__file__).resolve().parent / "data" / "Aston Villa.json"
...
def _load_source():
    ...
    if str(payload.get("equipo", "")).strip().casefold() != ASTON_VILLA.casefold():
        raise ValueError("Aston Villa.json no corresponde a Aston Villa")
    ...
    if len(players) != 23 or missing or extra:
        raise ValueError(
            f"Aston Villa.json inválido: jugadores={len(players)} "
            f"faltan={missing or '-'} sobran={extra or '-'}"
        )
    return by_name


ASTON_VILLA_DATA = _load_source()
...
def _role_values(position, stats):
    ...
    if values is None:
        raise RuntimeError(f"Posición Aston Villa sin fórmula OVR: {position}")
...
ASTON_VILLA_ROSTER = [
    (name, POSITION_BY_PLAYER[name], _rating(POSITION_BY_PLAYER[name], ASTON_VILLA_DATA[name]["stats"]))
    for name in POSITION_BY_PLAYER
]

# Make Aston Villa selectable before Discord views are registered.
if not any(name.casefold() == ASTON_VILLA.casefold() for name, _country in teams.OFFICIAL_TEAMS):
    teams.OFFICIAL_TEAMS.append((ASTON_VILLA, COUNTRY))
teams.OFFICIAL[ASTON_VILLA.casefold()] = ASTON_VILLA
...
def _sync_connection(runtime, conn):
    ...
    changed = 0
    for name, position, rating in ASTON_VILLA_ROSTER:
        row = conn.execute(
        ...
        if row:
            ...
            # Preserve the current club in case the player moved after Aston Villa was loaded.
            conn.execute(
            ...
            cursor = conn.execute(
                ...
                (name, position, ASTON_VILLA, rating, minimum_for_rating(rating)),
            )
            player_id = int(cursor.lastrowid)
        _upsert_attributes(conn, player_id, ASTON_VILLA_DATA[name])
        changed += 1
    ...
    if "deleted_teams" in _tables(conn):
        conn.execute("DELETE FROM deleted_teams WHERE name = ? COLLATE NOCASE", (ASTON_VILLA,))
    conn.execute("INSERT OR IGNORE INTO seed_state (key) VALUES (?)", (MIGRATION_MARKER,))
    return changed


def apply_aston_villa_json(runtime):
    if getattr(runtime, "_ajap_aston_villa_json", False):
        return
    ...
    def aston_villa_synced_db():
        ...
        try:
            ...
            if changed:
                print(
                    f"AJAP Aston Villa cargado: guild={guild_id} • "
                    f"{len(ASTON_VILLA_ROSTER)} jugadores desde Aston Villa.json"
                )
        except Exception:
            conn.close()
            raise
    ...
    runtime._ajap_aston_villa_json = True
    print(
        f"AJAP migración Aston Villa activa: {len(ASTON_VILLA_ROSTER)} jugadores • "
        "OVR AJPA de 3 stats"
    )


OVR_BY_PLAYER = {name: rating for name, _position, rating in ASTON_VILLA_ROSTER}
print(f"AJAP Aston Villa JSON listo: {len(ASTON_VILLA_ROSTER)} jugadores")
