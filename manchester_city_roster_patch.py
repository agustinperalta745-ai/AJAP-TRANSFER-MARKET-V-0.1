"""Load Manchester City from data/Manchester City.json with full PES6 stats and AJAP OVR."""
...
COUNTRY = "Inglaterra"
MIGRATION_MARKER = "manchester_city_json_v1_ovr3_20260827"
SOURCE = "Manchester City.json • OVR AJPA promedio de 3 stats • 2026-08-27"
SOURCE_PATH = Path(__file__).resolve().parent / "data" / "Manchester City.json"
...
def _load_source():
    ...
    if str(payload.get("equipo", "")).strip().casefold() != MANCHESTER_CITY.casefold():
        raise ValueError("Manchester City.json no corresponde a Manchester City")
    ...
    if len(players) != 28 or set(by_name) != set(POSITION_BY_PLAYER):
        ...
        raise ValueError(
            f"Manchester City.json inválido: jugadores={len(players)} faltan={missing or '-'} sobran={extra or '-'}"
        )
    return by_name


MANCHESTER_CITY_DATA = _load_source()
...
def _role_values(position, stats):
    ...
    if values is None:
        raise RuntimeError(f"Posición Manchester City sin fórmula OVR: {position}")
...
MANCHESTER_CITY_ROSTER = [
    (name, POSITION_BY_PLAYER[name], _rating(POSITION_BY_PLAYER[name], MANCHESTER_CITY_DATA[name]["stats"]))
    for name in POSITION_BY_PLAYER
]

# Make Manchester City visible in the team selector before Discord views are registered.
if not any(name.casefold() == MANCHESTER_CITY.casefold() for name, _country in teams.OFFICIAL_TEAMS):
    teams.OFFICIAL_TEAMS.append((MANCHESTER_CITY, COUNTRY))
teams.OFFICIAL[MANCHESTER_CITY.casefold()] = MANCHESTER_CITY
...
def _sync_connection(runtime, conn):
    ...
    names = [name for name, _position, _rating_value in MANCHESTER_CITY_ROSTER]
    marks = ",".join("?" for _ in names)
    present = conn.execute(
        f"SELECT COUNT(*) AS n FROM roster_players WHERE name COLLATE NOCASE IN ({marks})",
        tuple(names),
    ).fetchone()
    if int(present["n"] if present else 0) < len(MANCHESTER_CITY_ROSTER):
    ...
    if done:
        conn.execute(
            ...
            (MANCHESTER_CITY, COUNTRY),
        )
        return 0

    changed = 0
    for name, position, rating in MANCHESTER_CITY_ROSTER:
        row = conn.execute(
        ...
        if row:
            ...
            conn.execute(
                ...
                (position, MANCHESTER_CITY, rating, minimum_for_rating(rating), player_id),
            )
        else:
            cursor = conn.execute(
                ...
                (name, position, MANCHESTER_CITY, rating, minimum_for_rating(rating)),
            )
            player_id = int(cursor.lastrowid)
        _upsert_attributes(conn, player_id, MANCHESTER_CITY_DATA[name])
        changed += 1
    ...
    if "deleted_teams" in _tables(conn):
        conn.execute("DELETE FROM deleted_teams WHERE name = ? COLLATE NOCASE", (MANCHESTER_CITY,))
    conn.execute("INSERT INTO seed_state (key) VALUES (?)", (MIGRATION_MARKER,))
    return changed


def apply_manchester_city_json(runtime):
    if getattr(runtime, "_ajap_manchester_city_json", False):
        return
    base_db = runtime.db
    synced_guilds = set()

    def manchester_city_synced_db():
        conn = base_db()
        ...
        try:
            ...
            if changed:
                print(f"AJAP Manchester City cargado: guild={guild_id} • 28 jugadores desde Manchester City.json")
        except Exception:
            conn.close()
            raise
        return conn

    runtime.db = manchester_city_synced_db
    runtime._ajap_manchester_city_json = True
    print("AJAP migración Manchester City activa: 28 jugadores • OVR AJPA de 3 stats")


_original_apply_guild_isolation_patch = guild_isolation.apply_guild_isolation_patch


def _apply_guild_isolation_then_manchester_city(runtime, bot):
    _original_apply_guild_isolation_patch(runtime, bot)
    apply_manchester_city_json(runtime)


if not getattr(guild_isolation.apply_guild_isolation_patch, "_ajap_manchester_city_json_wrapped", False):
    _apply_guild_isolation_then_manchester_city._ajap_manchester_city_json_wrapped = True
    guild_isolation.apply_guild_isolation_patch = _apply_guild_isolation_then_manchester_city


OVR_BY_PLAYER = {name: rating for name, _position, rating in MANCHESTER_CITY_ROSTER}
print(f"AJAP Manchester City JSON listo: {len(MANCHESTER_CITY_ROSTER)} jugadores")
