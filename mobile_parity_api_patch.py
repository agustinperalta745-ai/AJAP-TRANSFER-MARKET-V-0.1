...
def league_payload(conn: sqlite3.Connection) -> dict:
    ...
    if "league_matches" in tables:
        ...
        for row in rows:
            ...
            result_cards.append({
                "id": int(row["id"]), "home_team": home_name, "away_team": away_name,
                "home_goals": hg, "away_goals": ag, "created_at": str(row["created_at"] or ""),
            })
            home["pj"] += 1; away["pj"] += 1
    ...
    if "league_goal_events" in tables:
        ...
        scorers = [
            {"player": str(row["player"]), "team": str(row["team"] or ""), "goals": int(row["goals"] or 0)}
            for row in rows if str(row["player"] or "").strip()
        ]
...
def history_payload(conn: sqlite3.Connection, limit: int = 100) -> dict:
    ...
    wanted = ["id", "player", "seller", "buyer", "amount", "status", "operation_type", "created_at", "approved_at", "applied_at", "notes"]
    selected = [name for name in wanted if name in cols]
    if not selected:
        return {"items": []}
    rows = conn.execute(f"SELECT {', '.join(selected)} FROM transfers ORDER BY id DESC LIMIT ?", (max(1, min(int(limit), 200)),)).fetchall()
    items = []
    for row in rows:
        keys = set(row.keys())
        items.append({
...
def assignments_payload(conn: sqlite3.Connection) -> dict:
    ...
    if not {"user_id", "name"}.issubset(cols):
        return {"assignments": []}
    rows = conn.execute("SELECT user_id, name FROM clubs WHERE user_id IS NOT NULL AND TRIM(COALESCE(name,''))<>'' ORDER BY name COLLATE NOCASE").fetchall()
    return {"assignments": [{"user_id": str(row["user_id"]), "club": str(row["name"])} for row in rows]}
...
def set_market_state(conn: sqlite3.Connection, session: dict, opened: bool) -> dict:
    ...
    conn.execute("""
        ...
        ON CONFLICT(id) DO UPDATE SET is_open=excluded.is_open, updated_by=excluded.updated_by, updated_at=CURRENT_TIMESTAMP
    """, (value, int(session["user_id"])))
    if old != value:
        conn.execute("INSERT INTO market_state_history (is_open, changed_by) VALUES (?, ?)", (value, int(session["user_id"])))
...
def apply_mobile_parity_api_patch() -> None:
    ...
    def get(self):
        ...
        try:
            if path == "/api/v1/league":
                with mobile_read_api.readonly_db() as conn: self._json(league_payload(conn))
                return
            if path == "/api/v1/history":
                with mobile_read_api.readonly_db() as conn: self._json(history_payload(conn))
                return
            if path == "/api/v1/admin/assignments":
                with mobile_write_api.write_db() as conn:
                    _staff_session(self.headers, conn); self._json(assignments_payload(conn))
                return
        except mobile_write_api.ApiFailure as exc:
            self._json({"error": "request", "message": exc.message}, exc.status); return
    ...
    def post(self):
        ...
        try:
            ...
            with mobile_write_api.write_db() as conn:
                mobile_write_api.ensure_schema(conn); session = _staff_session(self.headers, conn)
    ...
    handler.do_GET = get; handler.do_POST = post; handler.do_PUT = post; handler.do_PATCH = post
