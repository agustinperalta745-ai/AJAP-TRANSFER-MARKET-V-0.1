...
def league_payload(conn: sqlite3.Connection) -> dict:
    ...
    if "league_matches" in tables:
        ...
        for row in rows:
            ...
            result_cards.append({"id": int(row["id"]), "home_team": home_name, "away_team": away_name, "home_goals": hg, "away_goals": ag, "created_at": str(row["created_at"] or "")})
...
def history_payload(conn: sqlite3.Connection, limit: int = 100) -> dict:
    if "transfers" not in _tables(conn): return {"items": []}
...
def _staff_session(headers, conn: sqlite3.Connection) -> dict:
    ...
    if not session.get("is_staff"): raise mobile_write_api.ApiFailure("Esta herramienta es exclusiva para Staff.", HTTPStatus.FORBIDDEN)
    return session


def assignments_payload(conn: sqlite3.Connection) -> dict:
    if "clubs" not in _tables(conn): return {"assignments": []}
    cols = mobile_read_api._columns(conn, "clubs")
    if not {"user_id", "name"}.issubset(cols): return {"assignments": []}
