"""Load Fiorentina from data/Fiorentina.json with full PES6 stats and AJAP OVR."""
...
COUNTRY = "Italia"
MIGRATION_MARKER = "fiorentina_json_v1_ovr3_20260826"
SOURCE = "Fiorentina.json • OVR AJPA promedio de 3 stats • 2026-08-26"
SOURCE_PATH = Path(__file__).resolve().parent / "data" / "Fiorentina.json"

POSITION_BY_PLAYER = {
    "Giampaolo Pazzini": "CF",
    "Luca Toni": "CF",
    "Adrian Mutu": "WF/SS",
    "Guilherme Do Prado": "AMF/SS",
    "Massimo Riganò": "CF",
    "Reginaldo": "CF/WF",
    "Marco Donadel": "DMF/CMF",
    "Fabio Liverani": "DMF/CMF",
    "Mario Santana": "WF/RMF",
    "Martin Jorgensen": "LMF/RMF",
    "Manuele Blasi": "CMF/DMF",
    "Michele Pazienza": "DMF/CMF",
    "Massimo Gobbi": "LB/LMF",
    "Riccardo Montolivo": "DMF/CMF",
    "Maurizio Di Loreto": "CB",
    "Dario Dainelli": "CB",
    "Tomas Ujfalusi": "CB/RB",
    "Manuel Pasqual": "LB",
    "Alessandro Gamberini": "CB",
    "Per Kroldrup": "CB",
    "Alessandro Potenza": "RB/CB",
    "Sébastien Frey": "GK",
    "Bogdan Lobonț": "GK",
...
def _load_source():
    ...
    if str(payload.get("equipo", "")).strip().casefold() != FIORENTINA.casefold():
        raise ValueError("Fiorentina.json no corresponde a Fiorentina")
    players = payload.get("jugadores") or []
    by_name = {str(player["nombre"]).strip(): player for player in players}
    if len(players) != 25 or set(by_name) != set(POSITION_BY_PLAYER):
        missing = sorted(set(POSITION_BY_PLAYER) - set(by_name))
        extra = sorted(set(by_name) - set(POSITION_BY_PLAYER))
        raise ValueError(
            f"Fiorentina.json inválido: jugadores={len(players)} faltan={missing or '-'} sobran={extra or '-'}"
        )
    return by_name


FIORENTINA_DATA = _load_source()
...
def _role_values(position, stats):
    ...
    if values is None:
        raise RuntimeError(f"Posición Fiorentina sin fórmula OVR: {position}")
...
FIORENTINA_ROSTER = [
    (name, POSITION_BY_PLAYER[name], _rating(POSITION_BY_PLAYER[name], FIORENTINA_DATA[name]["stats"]))
    for name in POSITION_BY_PLAYER
]

if not any(name.casefold() == FIORENTINA.casefold() for name, _country in teams.OFFICIAL_TEAMS):
    teams.OFFICIAL_TEAMS.append((FIORENTINA, COUNTRY))
teams.OFFICIAL[FIORENTINA.casefold()] = FIORENTINA
...
def _sync_connection(runtime, conn):
    ...
    names = [name for name, _position, _rating_value in FIORENTINA_ROSTER]
    marks = ",".join("?" for _ in names)
    present = conn.execute(
        f"SELECT COUNT(*) AS n FROM roster_players WHERE name COLLATE NOCASE IN ({marks})",
        tuple(names),
    ).fetchone()
    if int(present["n"] if present else 0) < len(FIORENTINA_ROSTER):
    ...
    changed = 0
    for name, position, rating in FIORENTINA_ROSTER:
        row = conn.execute(
        ...
        if row:
            ...
            cursor = conn.execute(
                ...
                (name, position, FIORENTINA, rating, minimum_for_rating(rating)),
            )
            player_id = int(cursor.lastrowid)
        _upsert_attributes(conn, player_id, FIORENTINA_DATA[name])
        changed += 1
    ...
    if "deleted_teams" in _tables(conn):
        conn.execute("DELETE FROM deleted_teams WHERE name = ? COLLATE NOCASE", (FIORENTINA,))
    conn.execute("INSERT INTO seed_state (key) VALUES (?)", (MIGRATION_MARKER,))
    return changed


def apply_fiorentina_json(runtime):
    if getattr(runtime, "_ajap_fiorentina_json", False):
        return
    base_db = runtime.db
    synced_guilds = set()

    def fiorentina_synced_db():
        conn = base_db()
        ...
        try:
            ...
            if changed:
                print(f"AJAP Fiorentina cargada: guild={guild_id} • 25 jugadores desde Fiorentina.json")
        except Exception:
            conn.close()
            raise
        return conn

    runtime.db = fiorentina_synced_db
    runtime._ajap_fiorentina_json = True
    print("AJAP migración Fiorentina activa: 25 jugadores • OVR AJPA de 3 stats")


_original_apply_guild_isolation_patch = guild_isolation.apply_guild_isolation_patch


def _apply_guild_isolation_then_fiorentina(runtime, bot):
    _original_apply_guild_isolation_patch(runtime, bot)
    apply_fiorentina_json(runtime)


if not getattr(guild_isolation.apply_guild_isolation_patch, "_ajap_fiorentina_json_wrapped", False):
    _apply_guild_isolation_then_fiorentina._ajap_fiorentina_json_wrapped = True
    guild_isolation.apply_guild_isolation_patch = _apply_guild_isolation_then_fiorentina


OVR_BY_PLAYER = {name: rating for name, _position, rating in FIORENTINA_ROSTER}
print(f"AJAP Fiorentina JSON listo: {len(FIORENTINA_ROSTER)} jugadores")
