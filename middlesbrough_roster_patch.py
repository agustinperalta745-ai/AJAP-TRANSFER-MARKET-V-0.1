"""Load Middlesbrough from data/Middle.json with full PES6 stats and AJAP OVR."""
...
TEAM = "Middlesbrough"
COUNTRY = "Inglaterra"
MIGRATION_MARKER = "middlesbrough_json_v1_ovr3_20260827"
SOURCE = "Middle.json • OVR AJPA promedio de 3 stats • 2026-08-27"
SOURCE_PATH = Path(__file__).resolve().parent / "data" / "Middle.json"
...
def _load_source():
    ...
    if str(payload.get("equipo", "")).strip().casefold() != TEAM.casefold():
        raise ValueError("Middle.json no corresponde a Middlesbrough")
    players = payload.get("jugadores") or []
    by_name = {str(player["nombre"]).strip(): player for player in players}
    if len(players) != 32 or set(by_name) != set(POSITION_BY_PLAYER):
        missing = sorted(set(POSITION_BY_PLAYER) - set(by_name))
        extra = sorted(set(by_name) - set(POSITION_BY_PLAYER))
        raise ValueError(
            f"Middle.json inválido: jugadores={len(players)} faltan={missing or '-'} sobran={extra or '-'}"
        )
    return by_name


MIDDLESBROUGH_DATA = _load_source()
...
def _role_values(position, stats):
    ...
    if values is None:
        raise RuntimeError(f"Posición Middlesbrough sin fórmula OVR: {position}")
...
MIDDLESBROUGH_ROSTER = [
    (name, POSITION_BY_PLAYER[name], _rating(POSITION_BY_PLAYER[name], MIDDLESBROUGH_DATA[name]["stats"]))
    for name in POSITION_BY_PLAYER
]

if not any(name.casefold() == TEAM.casefold() for name, _country in teams.OFFICIAL_TEAMS):
    teams.OFFICIAL_TEAMS.append((TEAM, COUNTRY))
teams.OFFICIAL[TEAM.casefold()] = TEAM
...
def _sync_connection(runtime, conn):
    ...
    names = [name for name, _position, _rating_value in MIDDLESBROUGH_ROSTER]
    marks = ",".join("?" for _ in names)
    present = conn.execute(
        f"SELECT COUNT(*) AS n FROM roster_players WHERE name COLLATE NOCASE IN ({marks})",
        tuple(names),
    ).fetchone()
    if int(present["n"] if present else 0) < len(MIDDLESBROUGH_ROSTER):
    ...
    changed = 0
    for name, position, rating in MIDDLESBROUGH_ROSTER:
        row = conn.execute(
        ...
        if row:
            ...
            conn.execute(
                ...
                (position, TEAM, rating, minimum_for_rating(rating), player_id),
            )
        else:
            cursor = conn.execute(
                ...
                (name, position, TEAM, rating, minimum_for_rating(rating)),
            )
            player_id = int(cursor.lastrowid)
        _upsert_attributes(conn, player_id, MIDDLESBROUGH_DATA[name])
        changed += 1
    ...
    if "deleted_teams" in _tables(conn):
        conn.execute("DELETE FROM deleted_teams WHERE name = ? COLLATE NOCASE", (TEAM,))
    conn.execute("INSERT INTO seed_state (key) VALUES (?)", (MIGRATION_MARKER,))
    return changed


def apply_middlesbrough_json(runtime):
    if getattr(runtime, "_ajap_middlesbrough_json", False):
        return
    base_db = runtime.db
    synced_guilds = set()

    def middlesbrough_synced_db():
        conn = base_db()
        ...
        try:
            ...
            if changed:
                print(f"AJAP Middlesbrough cargado: guild={guild_id} • 32 jugadores desde Middle.json")
        except Exception:
            conn.close()
            raise
        return conn

    runtime.db = middlesbrough_synced_db
    runtime._ajap_middlesbrough_json = True
    print("AJAP migración Middlesbrough activa: 32 jugadores • OVR AJPA de 3 stats")


_original_apply_guild_isolation_patch = guild_isolation.apply_guild_isolation_patch


def _apply_guild_isolation_then_middlesbrough(runtime, bot):
    _original_apply_guild_isolation_patch(runtime, bot)
    apply_middlesbrough_json(runtime)


if not getattr(guild_isolation.apply_guild_isolation_patch, "_ajap_middlesbrough_json_wrapped", False):
    _apply_guild_isolation_then_middlesbrough._ajap_middlesbrough_json_wrapped = True
    guild_isolation.apply_guild_isolation_patch = _apply_guild_isolation_then_middlesbrough


OVR_BY_PLAYER = {name: rating for name, _position, rating in MIDDLESBROUGH_ROSTER}
print(f"AJAP Middlesbrough JSON listo: {len(MIDDLESBROUGH_ROSTER)} jugadores")
