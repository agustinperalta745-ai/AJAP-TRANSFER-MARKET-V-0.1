"""Load Bolton Wanderers from data/Bolton Wanderers.json with full PES6 stats and AJAP OVR."""
...
COUNTRY = "Inglaterra"
MIGRATION_MARKER = "bolton_wanderers_json_v1_ovr3_20260827"
SOURCE = "Bolton Wanderers.json • OVR AJPA promedio de 3 stats • 2026-08-27"
SOURCE_PATH = Path(__file__).resolve().parent / "data" / "Bolton Wanderers.json"
...
def _load_source():
    ...
    if str(payload.get("equipo", "")).strip().casefold() != BOLTON.casefold():
        raise ValueError("Bolton Wanderers.json no corresponde a Bolton Wanderers")
    players = payload.get("jugadores") or []
    by_name = {str(player["nombre"]).strip(): player for player in players}
    if len(players) != 21 or set(by_name) != set(POSITION_BY_PLAYER):
        missing = sorted(set(POSITION_BY_PLAYER) - set(by_name))
        extra = sorted(set(by_name) - set(POSITION_BY_PLAYER))
        raise ValueError(
            f"Bolton Wanderers.json inválido: jugadores={len(players)} faltan={missing or '-'} sobran={extra or '-'}"
        )
    return by_name


BOLTON_DATA = _load_source()
...
def _role_values(position, stats):
    ...
    if values is None:
        raise RuntimeError(f"Posición Bolton Wanderers sin fórmula OVR: {position}")
...
BOLTON_ROSTER = [
    (name, POSITION_BY_PLAYER[name], _rating(POSITION_BY_PLAYER[name], BOLTON_DATA[name]["stats"]))
    for name in POSITION_BY_PLAYER
]

# Make Bolton Wanderers visible in the team selector before Discord views are registered.
if not any(name.casefold() == BOLTON.casefold() for name, _country in teams.OFFICIAL_TEAMS):
    teams.OFFICIAL_TEAMS.append((BOLTON, COUNTRY))
teams.OFFICIAL[BOLTON.casefold()] = BOLTON
...
def _sync_connection(runtime, conn):
    ...
    names = [name for name, _position, _rating_value in BOLTON_ROSTER]
    marks = ",".join("?" for _ in names)
    present = conn.execute(
        f"SELECT COUNT(*) AS n FROM roster_players WHERE name COLLATE NOCASE IN ({marks})",
        tuple(names),
    ).fetchone()
    if int(present["n"] if present else 0) < len(BOLTON_ROSTER):
    ...
    changed = 0
    for name, position, rating in BOLTON_ROSTER:
        row = conn.execute(
        ...
        if row:
            ...
            conn.execute(
                ...
                (position, BOLTON, rating, minimum_for_rating(rating), player_id),
            )
        else:
            cursor = conn.execute(
                ...
                (name, position, BOLTON, rating, minimum_for_rating(rating)),
            )
            player_id = int(cursor.lastrowid)
        _upsert_attributes(conn, player_id, BOLTON_DATA[name])
        changed += 1
    ...
    if "deleted_teams" in _tables(conn):
        conn.execute("DELETE FROM deleted_teams WHERE name = ? COLLATE NOCASE", (BOLTON,))
    conn.execute("INSERT INTO seed_state (key) VALUES (?)", (MIGRATION_MARKER,))
    return changed


def apply_bolton_wanderers_json(runtime):
    if getattr(runtime, "_ajap_bolton_wanderers_json", False):
        return
    base_db = runtime.db
    synced_guilds = set()

    def bolton_wanderers_synced_db():
        conn = base_db()
        ...
        try:
            ...
            if changed:
                print(f"AJAP Bolton Wanderers cargado: guild={guild_id} • 21 jugadores desde Bolton Wanderers.json")
        except Exception:
            conn.close()
            raise
        return conn

    runtime.db = bolton_wanderers_synced_db
    runtime._ajap_bolton_wanderers_json = True
    print("AJAP migración Bolton Wanderers activa: 21 jugadores • OVR AJPA de 3 stats")


_original_apply_guild_isolation_patch = guild_isolation.apply_guild_isolation_patch


def _apply_guild_isolation_then_bolton_wanderers(runtime, bot):
    _original_apply_guild_isolation_patch(runtime, bot)
    apply_bolton_wanderers_json(runtime)


if not getattr(guild_isolation.apply_guild_isolation_patch, "_ajap_bolton_wanderers_json_wrapped", False):
    _apply_guild_isolation_then_bolton_wanderers._ajap_bolton_wanderers_json_wrapped = True
    guild_isolation.apply_guild_isolation_patch = _apply_guild_isolation_then_bolton_wanderers


OVR_BY_PLAYER = {name: rating for name, _position, rating in BOLTON_ROSTER}
print(f"AJAP Bolton Wanderers JSON listo: {len(BOLTON_ROSTER)} jugadores")
