"""Load Lazio from data/Lazio.json with full PES6 stats and AJAP OVR."""
...
LAZIO = "Lazio"
COUNTRY = "Italia"
MIGRATION_MARKER = "lazio_json_v1_ovr3_20260827"
SOURCE = "Lazio.json • OVR AJPA promedio de 3 stats • 2026-08-27"
SOURCE_PATH = Path(__file__).resolve().parent / "data" / "Lazio.json"
...
def _load_source():
    ...
    if str(payload.get("equipo", "")).strip().casefold() != LAZIO.casefold():
        raise ValueError("Lazio.json no corresponde a Lazio")
    ...
    if len(players) != 23 or set(by_name) != set(POSITION_BY_PLAYER):
        ...
        raise ValueError(
            f"Lazio.json inválido: jugadores={len(players)} faltan={missing or '-'} sobran={extra or '-'}"
        )
    return by_name


LAZIO_DATA = _load_source()
...
def _role_values(position, stats):
    ...
    if values is None:
        raise RuntimeError(f"Posición Lazio sin fórmula OVR: {position}")
...
LAZIO_ROSTER = [
    (name, POSITION_BY_PLAYER[name], _rating(POSITION_BY_PLAYER[name], LAZIO_DATA[name]["stats"]))
    for name in POSITION_BY_PLAYER
]

if not any(name.casefold() == LAZIO.casefold() for name, _country in teams.OFFICIAL_TEAMS):
    teams.OFFICIAL_TEAMS.append((LAZIO, COUNTRY))
teams.OFFICIAL[LAZIO.casefold()] = LAZIO
...
def _sync_connection(runtime, conn):
    ...
    names = [name for name, _position, _rating_value in LAZIO_ROSTER]
    marks = ",".join("?" for _ in names)
    present = conn.execute(
        f"SELECT COUNT(*) AS n FROM roster_players WHERE name COLLATE NOCASE IN ({marks})",
        tuple(names),
    ).fetchone()
    if int(present["n"] if present else 0) < len(LAZIO_ROSTER):
    ...
    changed = 0
    for name, position, rating in LAZIO_ROSTER:
        row = conn.execute(
        ...
        if row:
            ...
            conn.execute(
                ...
                (position, LAZIO, rating, minimum_for_rating(rating), player_id),
            )
        else:
            cursor = conn.execute(
                ...
                (name, position, LAZIO, rating, minimum_for_rating(rating)),
            )
            player_id = int(cursor.lastrowid)
        _upsert_attributes(conn, player_id, LAZIO_DATA[name])
        changed += 1
    ...
    if "deleted_teams" in _tables(conn):
        conn.execute("DELETE FROM deleted_teams WHERE name = ? COLLATE NOCASE", (LAZIO,))
    conn.execute("INSERT INTO seed_state (key) VALUES (?)", (MIGRATION_MARKER,))
    return changed


def apply_lazio_json(runtime):
    if getattr(runtime, "_ajap_lazio_json", False):
        return
    base_db = runtime.db
    synced_guilds = set()
    ...
    def lazio_synced_db():
        ...
        try:
            ...
            if changed:
                print(f"AJAP Lazio cargada: guild={guild_id} • 23 jugadores desde Lazio.json")
        except Exception:
            conn.close()
            raise
        return conn

    runtime.db = lazio_synced_db
    runtime._ajap_lazio_json = True
    print("AJAP migración Lazio activa: 23 jugadores • OVR AJPA de 3 stats")


_original_apply_guild_isolation_patch = guild_isolation.apply_guild_isolation_patch


def _apply_guild_isolation_then_lazio(runtime, bot):
    _original_apply_guild_isolation_patch(runtime, bot)
    apply_lazio_json(runtime)


if not getattr(guild_isolation.apply_guild_isolation_patch, "_ajap_lazio_json_wrapped", False):
    _apply_guild_isolation_then_lazio._ajap_lazio_json_wrapped = True
    guild_isolation.apply_guild_isolation_patch = _apply_guild_isolation_then_lazio


OVR_BY_PLAYER = {name: rating for name, _position, rating in LAZIO_ROSTER}
print(f"AJAP Lazio JSON listo: {len(LAZIO_ROSTER)} jugadores")
