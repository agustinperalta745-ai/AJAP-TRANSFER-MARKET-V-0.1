...
def panel_embed(user_id: int):
    ...
    embed = discord.Embed(
        ...
        description=(
            ...
            "Gestioná tu club, plantel, publicaciones y ofertas desde los botones. "
            "No necesitás memorizar comandos."
        ),
    )
    embed.add_field(
    ...
    embed.add_field(
        name="📤 Publicaciones",
        value="Disponibles durante toda la temporada",
        inline=False,
    )
...
def plantel_embed(club: str):
    ...
    embed = discord.Embed(title=f"🏟️ Plantel • {club}")
    if not jugadores:
        embed.description = "No hay jugadores cargados para este club."
    else:
        embed.description = "\n".join(
            f"**{j['position']}** • {j['name']}" for j in jugadores[:50]
        )
        embed.set_footer(text=f"{len(jugadores)} jugador(es) • Plantel oficial")
    return embed
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(
            embed=discord.Embed(
                ...
                description=f"Tu cuenta quedó vinculada a **{nombre}**.",
            ),
            ephemeral=True,
        )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        with db() as conn:
            cur = conn.execute(
                ...
                """,
                (
                    ficha["name"], ficha["position"], club, precio,
                    detalle, interaction.user.id,
                ),
            )
...
class PublicarSelect(discord.ui.Select):
    def __init__(self, jugadores):
        options = [
            discord.SelectOption(
                label=j["name"][:100],
                description=f"{j['position']} • {j['club']}"[:100],
                value=str(j["id"]),
            )
    ...
    async def callback(self, interaction: discord.Interaction):
        with db() as conn:
            ficha = conn.execute(
                "SELECT * FROM roster_players WHERE id = ?", (int(self.values[0]),)
            ).fetchone()
        if not ficha:
...
class BuscarJugadoresModal(discord.ui.Modal, title="Buscar jugadores"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        if self.precio_max.value.strip() and price_number(self.precio_max.value) is None:
            await interaction.response.send_message(
                "⚠️ El precio máximo debe ser un número.", ephemeral=True
            )
            return
        ...
        await interaction.response.send_message(
            embed=publicaciones_embed(
                resultados,
                f"🔎 Resultados de búsqueda ({len(resultados)})",
            ),
...
class OfertaModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if not comprador_club:
            await interaction.response.send_message(
                "⚠️ Primero registrá tu club desde **Mi club**.", ephemeral=True
            )
            return
        ...
        with db() as conn:
            cur = conn.execute(
                ...
                (
                    ...
                    interaction.user.id, comprador_club,
                    pub["owner_id"], pub["club"],
                ),
            )
...
class TransferiblesSelect(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        if not mercado_abierto():
            await interaction.response.send_message(
                "🔒 El mercado está cerrado. Podés ver los jugadores, pero todavía no ofertar.",
                ephemeral=True,
            )
            return
...
class OfertaDecisionView(discord.ui.View):
    ...
    async def aceptar(self, interaction: discord.Interaction, button: discord.ui.Button):
        if not mercado_abierto():
            await interaction.response.send_message(
                "🔒 El mercado está cerrado. La oferta queda congelada.", ephemeral=True
            )
            return
        ...
        if not pub or not ficha or ficha["club"].casefold() != oferta["to_club"].casefold():
            ...
            await interaction.response.send_message(
                "⚠️ La publicación o propiedad cambió. La oferta fue cancelada.",
                ephemeral=True,
            )
            return
        ...
        with db() as conn:
            ...
            cur = conn.execute(
                ...
                """,
                (
                    oferta["player"], oferta["to_club"],
                    oferta["from_club"], oferta["amount"], oferta["id"],
                ),
            )
        ...
        embed = discord.Embed(
            ...
            description=f"**{oferta['player']}** pasó a **{oferta['from_club']}**.",
        )
        embed.add_field(name="Monto", value=oferta["amount"], inline=True)
        embed.set_footer(text=f"Transferencia #{transferencia_id}")
...
class AdminAgregarModal(discord.ui.Modal, title="Agregar jugador"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if existente:
            await interaction.response.send_message(
                f"⚠️ Ya pertenece a **{existente['club']}**.", ephemeral=True
            )
            return
        with db() as conn:
            conn.execute(
                ...
                (
                    self.jugador.value.strip(), self.posicion.value.strip().upper(),
                    self.club.value.strip(), interaction.user.id,
                ),
            )
        await interaction.response.send_message(
            f"✅ **{self.jugador.value.strip()}** agregado a **{self.club.value.strip()}**.",
            ephemeral=True,
        )


class AdminMoverModal(discord.ui.Modal, title="Mover jugador"):
    jugador = discord.ui.TextInput(label="Jugador", max_length=60)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(
            f"✅ **{ficha['name']}** movido de **{ficha['club']}** a **{destino}**.",
            ephemeral=True,
        )
...
class AdminQuitarModal(discord.ui.Modal, title="Quitar jugador"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(
            f"🗑️ **{ficha['name']}** eliminado de los planteles.", ephemeral=True
        )
...
class AdminPlantelModal(discord.ui.Modal, title="Consultar plantel"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        if not es_admin(interaction):
            ...
            return
        await interaction.response.send_message(
            embed=plantel_embed(self.club.value.strip()), ephemeral=True
        )
...
class AdminView(discord.ui.View):
    ...
    @discord.ui.button(label="Mover jugador", emoji="🔁", style=discord.ButtonStyle.primary, row=1)
...
class MercadoView(discord.ui.View):
    ...
    async def mi_club(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if not club:
            ...
            return
        await interaction.response.send_message(
            embed=plantel_embed(club), view=ClubView(), ephemeral=True
        )
    ...
    async def publicar(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        jugadores = [
            ...
            if not publicacion_activa_del_jugador(j["name"])
        ]
        if not jugadores:
            await interaction.response.send_message(
                "⚠️ No tenés jugadores disponibles para publicar. Puede que tu plantel esté vacío o que todos ya estén publicados.",
                ephemeral=True,
            )
            return
        await interaction.response.send_message(
            "📤 Elegí el jugador que querés publicar:",
            view=PublicarView(jugadores[:25]),
            ephemeral=True,
        )
    ...
    async def transferibles(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        await interaction.response.send_message(
            embed=publicaciones_embed(pubs),
            view=TransferiblesView(pubs),
            ephemeral=True,
        )
    ...
    async def ofertas(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        await interaction.response.send_message(
            embed=embed, view=OfertasView(pendientes), ephemeral=True
        )
    ...
    async def transferencias(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_message(
            embed=transferencias_embed(), ephemeral=True
        )
    ...
    async def administracion(self, interaction: discord.Interaction, button: discord.ui.Button):
        if not es_admin(interaction):
            await interaction.response.send_message(
                "⛔ Este menú es solo para administradores.", ephemeral=True
            )
            return
        embed = discord.Embed(
            title="⚙️ Administración",
            description=(
                "Desde acá podés abrir/cerrar el mercado y gestionar los planteles oficiales."
            ),
        )
        await interaction.response.send_message(
...
async def mercado(interaction: discord.Interaction):
    await interaction.response.send_message(
        embed=panel_embed(interaction.user.id), view=MercadoView()
    )
