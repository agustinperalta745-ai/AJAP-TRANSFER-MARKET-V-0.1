...
def apply_mobile_auth_patch() -> None:
    ...
    def json_with_auth_header(self, payload, status=HTTPStatus.OK):
        # Native Android does not require CORS, but this keeps the endpoint ready
        # for a future web client. The original serializer still owns the body.
        return original_json(self, payload, status)

    handler.do_GET = authenticated_get
    handler._json = json_with_auth_header
