    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if not interaction.guild:
            await interaction.response.send_message(
                "⚠️ Esta herramienta solo funciona dentro del servidor.",
                ephemeral=True,
            )
            return
        ...
        if getattr(member, "bot", False):
            await interaction.response.send_message(
                "⚠️ No se puede asignar un club a un bot.",
                ephemeral=True,
            )
            return
        ...
        if not available:
            await interaction.response.send_message(
                "⚠️ No hay equipos JSON libres para asignar.",
                ephemeral=True,
            )
            return
